...
sym_data = TFF.generate_fields(
    ...
    size=500,
    mode="cGL",
    info={"a": 0.5, "b": 0.1, "dx": 1, "dt": 0.0001},
)
#%%
ratio = 0.4
data_noise = []
...
#%%
result_d0 = TFF.kalman_FFTC(
        data=train_noise[0:200], mode=0, noise=1e-2, use_em=False
    )
result_d1 = TFF.kalman_FFTC(
        data=train_noise[0:200], mode=1, noise=1e-2, use_em=False
    )
result = [x.fftc() for x in train_noise]

#%%
dic = {}
for noise in [1e-5, 1e-2, 1, 1e2, 1e5]:
    result_d0 = TFF.kalman_FFTC(
        data=train_noise[0:400], mode=1, noise=noise, use_em=False
    )
...
train_noise = [x.inv_fftc(noise_ratio=ratio) for x in data_noise]
#%%%
mean = np.zeros((train_noise[0].shape[0],))
cov = np.eye(train_noise[0].shape[0]) * df.loc[:, "vx"].abs().mean() * ratio
cov_y= np.eye(train_noise[0].shape[0]) * df.loc[:, "vy"].abs().mean() * ratio
